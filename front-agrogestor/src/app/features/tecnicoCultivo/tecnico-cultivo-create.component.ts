// src/app/features/tecnicoCultivo/tecnico-cultivo-create.component.ts
...
@Component({
  standalone: true,
  selector: 'app-tecnico-cultivo-create',
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './tecnico-cultivo-create.component.html',
})
export class TecnicoCultivoCreateComponent implements OnInit, AfterViewInit {
  // parcelas solo del técnico
  parcelas: Parcela[] = [];
  ...
  constructor(
    ...
    // configurar iconos Leaflet desde assets
    ...
    this.authSvc.me().subscribe({
      next: (user: Usuario) => {
        ...
        this.parcelaSvc.getAll().subscribe({
          next: list => {
            this.parcelas = list.filter(p => p.usuario.id === this.currentUserId);
            this.filteredParcelas = [...this.parcelas];
          },
          error: () => {
            this.error = 'No se pudieron cargar tus parcelas';
          }
        });
      },
      error: () => {
        this.error = 'No se pudo autenticar al usuario';
      }
    });
  }
  ...
  selectParcela(p: Parcela): void {
    ...
    this.parcelaSearch = `${p.nombre} — ${p.propietario}`;
